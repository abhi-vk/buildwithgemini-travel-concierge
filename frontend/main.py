...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            txt = root.text
            if not txt.startswith("'Context' object has no attribute"):
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        # Non-streaming fallback: pull parts from task.history and final task artifacts.
        if last_task is not None:
            if getattr(last_task, "history", None):
                for m in last_task.history:
                    if str(getattr(m, "role", "")).endswith("agent"):
                        parts.extend(_extract_parts(getattr(m, "parts", [])))
